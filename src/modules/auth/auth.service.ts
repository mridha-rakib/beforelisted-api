...
const AGENT_REGISTRATION_ADMIN_EMAIL = SYSTEM_DEFAULT_ADMIN.email;
const AGENT_REGISTRATION_SUPPORT_EMAIL = "support@beforelisted.com";
const AGENT_REGISTRATION_ALLOWED_ADMIN_EMAILS = new Set([
  AGENT_REGISTRATION_ADMIN_EMAIL.toLowerCase(),
  AGENT_REGISTRATION_SUPPORT_EMAIL.toLowerCase(),
]);
...
export class AuthService {
  ...
  async verifyEmail(payload: VerifyEmailPayload): Promise<{
    ...
    logger.info(
      { userId: result.userId, userType: result.userType, verifiedRole },
      "Email verified and user marked",
    );

    if (verifiedRole === ROLES.AGENT) {
      const agentRepository = new AgentProfileRepository();
      ...
      const adminEmail = this.resolveAgentRegistrationAdminEmail();
      ...
      const agentBrokerageForAdminEmail = agentProfile?.brokerageName || "N/A";
      const agentPhoneNumber = user.phoneNumber || "N/A";
      const agentRegistrationLink
        = user.referralLink
          || (user.referralCode
      ...
      try {
        ...
        if (!emailResult.success) {
          logger.warn(
            ...
            "Agent registration verified admin email failed to send",
          );
        }
      }
      catch (error) {
    ...
    await this.emailService.sendWelcomeEmail({
      ...
      userType: verifiedRole,
      loginLink: `${env.CLIENT_URL}/login`,
      isPasswordAutoGenerated: user.passwordAutoGenerated,
      temporaryPassword: undefined,
      registeredAgent:
        verifiedRole === ROLES.RENTER ? renterRegisteredAgent : undefined,
    });

    if (
      verifiedRole === ROLES.RENTER
    ...
    else if (verifiedRole === ROLES.RENTER) {
      logger.info(
        {
          userId: user._id,
          email: user.email,
  ...
  private normalizeVerifiedRole(
    otpUserType: string,
    currentUserRole: string,
  ): string {
    const normalizedOtpUserType = otpUserType?.trim().toLowerCase();
    const normalizedCurrentUserRole = currentUserRole?.trim().toLowerCase();

    if (
      normalizedOtpUserType === ROLES.AGENT.toLowerCase()
      || normalizedCurrentUserRole === ROLES.AGENT.toLowerCase()
    ) {
      return ROLES.AGENT;
    }

    if (
      normalizedOtpUserType === ROLES.RENTER.toLowerCase()
      || normalizedCurrentUserRole === ROLES.RENTER.toLowerCase()
    ) {
      return ROLES.RENTER;
    }

    return currentUserRole || otpUserType;
  }

  private resolveAgentRegistrationAdminEmail(): string {
    const configuredAdminEmail = env.ADMIN_EMAIL?.trim();

    if (
      configuredAdminEmail
      && AGENT_REGISTRATION_ALLOWED_ADMIN_EMAILS.has(
    ...
    if (configuredAdminEmail) {
      logger.warn(
        {
          configuredAdminEmail,
          fallbackAdminEmail: AGENT_REGISTRATION_ADMIN_EMAIL,
        },
        "Using BeforeListed admin inbox for agent registration verified email",
      );
    }

    return AGENT_REGISTRATION_ADMIN_EMAIL;
  }
