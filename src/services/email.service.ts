...
export class EmailService {
  ...
  async sendRenterSearchReactivatedAgentNotification(
    ...
    try {
      ...
      const agentFirstName =
        payload.agentName?.trim().split(/\s+/)[0] ||
        payload.agentName ||
        "Agent";
  ...
  async sendAdminReferralEmail(
    ...
    try {
      ...
      if (
        ...
        _passwordExpiryTime = payload.passwordExpiresAt.toLocaleString(
          "en-US",
          {
            year: "numeric",
            month: "short",
...
<body>
  ...
  async sendAgentUnmatchedNotification(
    ...
    try {
      const subject = `Update on Your BeforeListed Match — ${payload.renterFirstName} ${payload.renterLastInitial}.`;

      const personalMessageBlock =
        payload.personalMessage && payload.personalMessage.trim().length > 0
          ? `
  ...
  async sendRequestUpdateToMatchedAgent(
    ...
    try {
      const subject = `Update Requested by ${payload.registeredAgentFullName} on ${payload.renterFirstName} ${payload.renterLastInitial}. | BeforeListed\u2122`;
